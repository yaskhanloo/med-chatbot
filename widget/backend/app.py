"""
Medical Chatbot Backend API with vLLM
Flask REST API 
"""
...
# Choose your Phi model:
# - "microsoft/phi-3.5-mini-instruct" (3.8B, recommended)
# - "microsoft/phi-3-mini-4k-instruct" (3.8B, older)
# - "microsoft/Phi-3-medium-4k-instruct" (14B, more capable)
model_handler = ModelHandler(model_id="microsoft/phi-3.5-mini-instruct")
...
def chat():
    ...
    try:
        ...
        max_tokens = data.get('max_tokens', 300)  # Allow client to specify
        ...
        # vLLM is fast, so this should be quick!
...
def clear_conversation_endpoint():
    ...
    try:
        clear_conversation()
        model_handler.reset_conversation()  # Also reset model handler state
        return jsonify({
            'status': 'success',
